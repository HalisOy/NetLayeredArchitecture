...
public class ClaimManager : IClaimService
{
    ...
    {
        var claim = _claimRepository.Get(u => u.Id == id);
    ...
    {
        return _claimRepository.GetAll(predicate: claim => claim.UserClaim.UserId == userId, include: claim => claim.Include(c => c.UserClaim)).ToList();
    }

    public async Task<IList<Claim>> GetAllByUserIdAsync(Guid userId)
    {
        var result = await _claimRepository.GetAllAsync(predicate: claim => claim.UserClaim.UserId == userId, include: claim => claim.Include(c => c.UserClaim));
        return result.ToList();
    }
